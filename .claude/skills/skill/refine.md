...
6. **Agnostic dependency issues** — hardcoded CLI tools or service names for external operations, missing fallback sections (see `./references/skill-guidelines.md` § Agnostic External Dependencies)
7. **Shell preprocessing opportunities** — bash blocks with static commands that could be preprocessed (see `./references/skill-guidelines.md` § Shell Preprocessing)
